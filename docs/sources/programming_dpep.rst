...
Primitive types, such as Python and Numpy ``float``, ``int``, or ``complex``, used to represent scalars,
...
The ``dpnp`` library is a bare minimum to start programming numerical codes for the data-parallel devices.

If you already have a Python script written in `Numpy*`_, you may need to change a few lines of the code to execute it:
...
with integrated or discrete GPU (it is a ``"host"`` on systems that do not have GPU).
...
All ``dpnp`` array creation routines and random number generators have optional keyword arguments:
...
In this example, you can see a custom function ``sum_it()`` that takes an array input. It is compiled with
...
associated with the queue, and on exit from the offload kernel it is assigned to the tensor y.

The ``parallel=True`` setting in ``@njit`` is essential to enable generation of data parallel kernels.

.. note:: ``fastmath=True`` is used in ``@njit`` decorator. This is an important setting to instruct the compiler that you are okay with NOT preserving the order of floating-point operations. It enables generation of instructions, such as SIMD, for a greater performance.
...
3. **Zero-copy data exchange between tensor implementations.** Certain Python projects may have their tensor
   implementations not relying on ``dpctl.tensor`` or ``dpnp.ndarray`` tensors. However, you still exchange data
...
Intel(R) VTune* Profiler examines various performance aspects of a program like, the most time-consuming parts,
efficiency of offloaded code, impact of memory sub-systems, etc.

Intel(R) Advisor* provides insights on the performance of offloaded code related to the peak performance and
memory bandwidth.

Below you can find how to use Intel(R) VTune* Profiler and Intel(R) Advisor* with
heterogenous programs that use **Data Parallel Extensions for Python**.

Profiling with Intel(R) VTune* Profiler
---------------------------------------
...
The following instructions are used to execute two Intel(R) VTune* Profiler analyses on programs written
...
Intel(R) VTune* Profiler performs dynamic binary analysis on a given program to obtain insights on various
performance characteristics. It can run on unmodified binaries with no extra requirements for program compilation.
After collecting the data using the above commands, the Intel(R) VTune* Profiler GUI can be used to view various
...
See `Intel VTune Profiler User Guide <https://www.intel.com/content/www/us/en/develop/documentation/vtune-help/top.html>`_
for more details. 

Profiling with Intel(R) Advisor*
--------------------------------
...
to analyze the performance of GPU kernels against the hardware roof-line performance. Intel |copy| Advisor* gives you
information about the maximum achievable performance with the given hardware conditions and helps identify the best
kernels for optimization. Further, it helps you characterize if a GPU kernel is bound by
compute capacity or by memory bandwidth.

The following instructions are used to generate GPU roof-line performance graphs using Intel(R) Advisor*:
...
The figure above shows the example of roof-line graph generated using Intel(R) Advisor*.
...
double precision or half-precision. 

**Data Parallel Extensions for Python** select default ``dtype`` depending on
default type of device in accordance with Python Array API Standard. It can be ``float64`` or ``float32``.
...
Situations with catastrophic cancellations must be carefully handled. 

An example where catastrophic
...
2. Run your code on a representative set of inputs in single and double precisions.
   ...
   If results remain identical to the three-five digits for different inputs, your code
   is not sensitive to floating-point errors.

3. Write your code with catastrophic cancellations in mind. These blocks of code require special
